...
public class AdministratorController {
	...
	public String addStudentClass(@Valid @ModelAttribute("studentclass-form") StudentClassForm form,
		...
		if (result.hasErrors()) {
			...
			if (scDao.isExist(sc)) {
				...
				scDao.save(sc);
				return "redirect:/administrator/studentclass?page=1&max=10";
			}
		}
	...
	public String assignStudentClassDelete(@PathVariable("iduser") Long iduser, @PathVariable("idclass") Long idclass,
			Model model) {
		try {
	...
	public String assignStudentClassAdd(@PathVariable("iduser") Long iduser, @PathVariable("idclass") Long idclass,
			Model model) {
		try {
			StudentClass sc = scDao.findById(idclass);
			User u = userDao.findById(iduser);
...
//				}
	...
	public String configureSmtp(@Valid @ModelAttribute("email-form") EmailForm form, BindingResult result,
			Model model) {
		if (result.hasErrors()) {
			model.addAttribute("errors", result);
			return "administrator/configureSmtp";
	...
	@GetMapping("/administatorDashboard")
	public String viewAdminDashboard(Model model) {
		model.addAttribute("countStudent",userDao.countByType(UserType.STUDENT));
		model.addAttribute("countProfessor",userDao.countByType(UserType.PROFESSOR));
		model.addAttribute("countAdmin",userDao.countByType(UserType.ADMINISTRATOR));
		model.addAttribute("countStudentClass",scDao.count());
		model.addAttribute("countQuiz",quizDao.count());
		return "administrator/adminDasboard";
	}
